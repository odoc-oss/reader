...
当后台服务不可用时，可以使用离线模式查看前端渲染效果：

```bash
# Web 离线模式
bun run dev:offline

# Tauri 离线模式（首次运行前需要先构建前端）
# 1. 首次运行前，先构建前端以创建 dist 目录
bun run build

# 2. 然后启动 Tauri 离线模式
bun run tauri:dev:offline
```

**离线模式特性：**
- ✅ 跳过所有登录验证，使用 mock 用户数据
- ✅ 可以访问所有需要认证的页面（文库、笔记等）
- ⚠️ 所有 API 调用都会失败（但不会阻止页面渲染）
- ⚠️ 仅用于前端 UI 开发和调试

**注意事项：**
- Tauri 离线模式需要 `dist` 目录存在，首次运行前必须先执行 `bun run build`
- 如果遇到 `resource path '../dist' doesn't exist` 错误，说明需要先构建前端
