...
export const getAPIRoot = () => {
  if (isTauriMode()) {
    // Tauri 开发模式：直接请求后端，绕过 Vite 代理（支持大文件上传）
    if (import.meta.env.DEV) {
      return 'http://localhost:8081/api';
    }
    // Tauri Release 模式：使用相对路径，由 gateway 代理
    return '/api';
  }
...
function throwLoginError(response: AxiosResponse) {
  ...
  if (!data) return; // 空 body（如 Content-Length: 0）直接跳过
  // 后端有时用 status 字段，有时用 code 字段，两者都检查
  ...
  if (errorCode === ERROR_CODE_TOKEN_EXPIRED) {
    ...
    errorCode === ERROR_CODE_ILLEGAL_TOKEN ||
    errorCode === ERROR_CODE_ILLEGAL_IDENTITY_REQUEST_REFRESH ||
    errorCode === ERROR_CODE_EXPIRED_IDENTITY_REQUEST_REFRESH ||
    errorCode === ERROR_CODE_REPEAT_LOGIN
  ) {
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // 对所有有响应体的 HTTP 错误都尝试 login 错误处理，不只限于 401
    if (response) {
      throwLoginError(response);
    }
    ...
    if (!error.config?.headers[HEADER_CANCLE_AUTO_ERROR]) {
      message.error(backendMessage || error.message || UNKNOWN_ERROR_MESSAGE);
    }

    throw new ResponseError({
      code,
      message: backendMessage || error.message || UNKNOWN_ERROR_MESSAGE,
    });
  }
);
