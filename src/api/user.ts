import { MessageTypeEnum } from '@odoc/types-go-sea-proto/types/cn/edu/idea/cloud/aiKnowledge/operator/admin/msg/SystemMsgInfo';
...
import { IsDisplayVipAdjustContentResponse } from '@odoc/types-go-sea-proto/types/cn/edu/idea/cloud/aiKnowledge/MessageCenter';
import {
  GetUserRedDotListResponse,
  ClearRedDotReq,
} from '@odoc/types-go-sea-proto/types/cn/edu/idea/cloud/aiKnowledge/RedDot';

import { User } from 'go-sea-proto/gen/ts/user/User'
import { GetMyStatisticalReps } from '@odoc/types-go-sea-proto/types/cn/edu/idea/cloud/aiKnowledge/homePage/StatisticalInformation'

// import { GetTrafficReq, GetTrafficResp } from '@odoc/types-go-sea-proto/types/cn/edu/idea/cloud/ab/ABTestInfo';
...
export const checkCloudLoginStatus = async (): Promise<boolean> => {
  try {
    const res = await api.get<SuccessResponse<CloudLoginStatusResponse>>(
      `/system/cloudLoginStatus`,
      getHeadersWithCancelAutoError()
    );
    return res.data.data?.valid ?? false;
  } catch (error) {
    console.error('[checkCloudLoginStatus] 检查云端登录状态失败:', error);
    return false;
  }
};
