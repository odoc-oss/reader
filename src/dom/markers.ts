// import {
  ...
  // PdfBBox,
// } from '@odoc/types-go-sea-proto/types/cn/edu/idea/cloud/pdf/PdfParse';
...
import { ViewerEvent, ViewerController } from '@odoc/pdf-annotate-viewer';
import {
...
export default function buildMarkers(
  ...
  const fetchParseData = async () => {
    ...
    if (!useServiceOffStore().isServiceOn) {
      const store = useStore();
      store.commit(
        `parse/${ParseMutationTypes.SET_PDF_FIGURE_AND_TABLE_INFOS}`,
        { pdfId, infos: { list: [], list0: [], pending: false, error: null } }
      );
      return;
    }
