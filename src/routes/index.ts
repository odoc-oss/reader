  ...
  const rules = [
    {
      ...
      children: [
        ...
        },
        {
          path: 'importdata',
          // 这里需要指定用于“数据导入”页面的组件
          component: () => import('@/pages/ImportData.vue'), // 假设您的页面组件路径是这个
          name: PAGE_ROUTE_NAME.IMPORT_DATA_IN_WORKBENCH, // 建议在 type.ts 中定义一个新常量
          meta: { auth: true, title: i18n.global.t('workbench.importData') },
        }
      ],
...
function createVueRouter() {
  ...
  router.beforeEach(async (to) => {
    ...
    // 如果路由需要认证，检查用户是否已登录
    // 离线模式下跳过认证检查（mock 用户已在 startup 中注入）
    if (requiresAuth && !isOfflineMode()) {
      try {
