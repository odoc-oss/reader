...
async function selectFilesWithTauri(options: FileSelectOptions): Promise<FileSelectResult[]> {
  console.log('[fileSelector] selectFilesWithTauri called, options:', options);
  const dialogApi = await getTauriDialogApi();
  
  if (!dialogApi) {
    console.warn('[fileSelector] Tauri dialog API not available, falling back to browser mode');
    return selectFilesWithBrowser(options);
  }
  
  console.log('[fileSelector] dialogApi loaded, calling open...');
  try {
    ...
    console.log('[fileSelector] dialog.open result:', selected, 'type:', typeof selected);
    ...
    console.log('[fileSelector] selected paths:', paths);
    ...
    console.error('[fileSelector] dialog.open failed:', error);
    throw error;
  }
}
