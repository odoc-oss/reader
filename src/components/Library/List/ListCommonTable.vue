    ...
    >
      ...
      <template #overlay>
        <Menu v-model:open-keys="openKeys">
          <Item
            key="rename"
            @click="renameDocId = item.docId"
            @mouseenter="handleOpen('', false)"
            >{{ $t('home.global.rename') }}</Item
          >
          <Divider />
          <Item
            key="open"
          ...
          <Item v-if="serviceOffStore.isServiceOn" key="cite" @mouseenter="handleOpen('', false)">
            <Cite
              class="share"
...
import { UserDocClassifyInfo } from '@idea/types-go-sea-proto/types/cn/edu/idea/cloud/userCenter/UserDoc'
import { UserDocInfo } from '@idea/types-go-sea-proto/types/cn/edu/idea/cloud/doc/UserDocManage'
...
provide('renameLibraryDoc', renameDocId)
