      ...
      >
        ...
        <div v-if="column.key === 'docName'" class="title">
          <div v-if="!isEdit" class="edit-filename">
            ...
            />
            <Tooltip :title="item.docName" placement="top">
              <p class="mb-0 text">
                <Tag
        ...
        <div v-else-if="column.key === 'parseProgress'" class="parse-progress-cell">
          ...
          <ReloadOutlined 
            v-if="serviceOffStore.isServiceOn && shouldShowReparseIcon(item)"
...
import { StyleValue, computed, ref, nextTick, onUnmounted, inject, watch, Ref } from 'vue'
...
const renameDocId = inject<Ref<string>>('renameLibraryDoc', ref(''))
watch(renameDocId, (id) => {
  if (id === props.item.docId) {
    renameDocId.value = ''
    nextTick(() => startEditFileName())
  }
})

const onPencilClick = (e: MouseEvent) => {
  e.stopPropagation()
  const rowEl = (e.currentTarget as HTMLElement).closest('.literature-list-item')
  if (rowEl) {
...
.literature-list-item {
  ...
  &.ant-dropdown-open .title .iconedit {
    opacity: 1 !important;
  }
