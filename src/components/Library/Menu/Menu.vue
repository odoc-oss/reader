        ...
        >
          <span v-if="key === allKey" class="root-folder" :data-key="key">
            ...
            <Dropdown
              :trigger="['click']"
              :get-popup-container="getPopupContainer"
              class="tree-pencil-dropdown root-pencil-dropdown"
            >
              <span class="tree-pencil-btn" @click.stop>
                <EditOutlined />
              </span>
              <template #overlay>
                <Menu>
                  <Item @click="showFileUploader">{{ $t('home.library.addLiterature') }}</Item>
                  <Item @click="allAddChildFolderStart">{{ $t('home.library.addFolder') }}</Item>
            ...
            <span class="dis-flex" :class="{ isLeaf }" :data-key="key" @mousedown="onMouseDown(key)"
              ...
              <div class="drop-middle" :class="{
                ...
                <Dropdown
                  ...
                  :trigger="['click']"
                  :get-popup-container="getPopupContainer"
                  class="tree-pencil-dropdown"
                >
                  <span class="tree-pencil-btn" @click.stop="storeLibraryIndex.selectedKey = key">
                    <EditOutlined />
                  </span>
                  <template #overlay>
                    <Menu>
                      ...
                      <Item :title="$t('home.library.moveTo')">
                        <SubMenu
                          v-if="storeLibraryIndex.selectedKey === key"
                      ...
                      <Item
                        v-if="storeLibraryIndex.libraryIndexExtra[key] && storeLibraryIndex.libraryIndexExtra[key].path.length >= 2"
                      ...
                      <Divider />
                      <Item class="del-doc" @click="removeModalVisible = true">{{ $t('home.library.delLiterature') }}</Item>
                    </Menu>
                  </template>
                </Dropdown>
                <Dropdown
                  v-else-if="!isLeaf && !isEditing"
                  :trigger="['click']"
                  :get-popup-container="getPopupContainer"
                  class="tree-pencil-dropdown"
                >
                  ...
                  <template #overlay>
                    <Menu>
                      <Item @click="showFileUploader">{{ $t('home.library.addLiterature') }}</Item>
                      <Item @click="renameStart">{{ $t('home.global.rename') }}</Item>
                      <Item @click="addSiblingFolderStart">{{ $t('home.library.addFolder') }}</Item>
                      <Item @click="addChildFolderStart">{{ $t('home.library.addSubFolder') }}</Item>
                      <Item :title="$t('home.library.moveTo')">
                        <SubMenu
                          v-if="storeLibraryIndex.selectedKey === key"
                          :list="storeLibraryIndex.libraryIndexList"
                      ...
                      <Divider />
                      <Item class="del-folder" @click="removeModalVisible = true">{{ $t('home.library.delFolder') }}</Item>
                    </Menu>
                  </template>
                </Dropdown>
              </div>
          ...
          <template #overlay>
            ...
            <Menu v-else-if="isLeaf">
              ...
              <Item v-if="serviceOffStore.isServiceOn">
                <Cite :paper-id="paperId" :pdf-id="pdfId" :page-type="PageType.library">
                  {{ $t('home.paper.quote') }}
                </Cite>
              </Item>
...
import { liteThrottle } from '@odoc/aiknowledge-special-util'
