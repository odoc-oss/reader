<template>
  <a-spin :spinning="loading" class="cite-dialog">
...
import { MyCslItem } from 'go-sea-proto/gen/ts/doc/CSL'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const activeKey = ref<bigint | string>('')
    ...
    const onTabChange = (key: bigint | string) => {
      activeKey.value = key
    }
    ...
    // 调试：打印显示条件的各个值
    const debugShowCondition = computed(() => {
      const hasMetaData = !!metaData.value
      const hasStyleList = !!showTabCitationStyleList.value
      const styleListLength = showTabCitationStyleList.value?.length || 0
      const isNotLoading = !loading.value
      const finalResult = hasMetaData && hasStyleList && styleListLength > 0 && isNotLoading
      
      console.log('[CitePane] 显示条件调试:', {
        'metaData存在': hasMetaData,
        'showTabCitationStyleList存在': hasStyleList,
        'showTabCitationStyleList.length': styleListLength,
    ...
    // 监听显示条件变化
    watch(debugShowCondition, (newVal) => {
      console.log('[CitePane] 显示条件变化 - 最终结果:', newVal)
    })

    onMounted(() => {
      // 触发一次调试输出
      console.log('[CitePane] onMounted - 初始显示条件:', debugShowCondition.value)
