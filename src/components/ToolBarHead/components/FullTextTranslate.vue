  ...
  >
    ...
    <button
      ...
      :class="[
        ...
        {
          'active': !closed,
          'service-off': !serviceOffStore.isServiceOn,
        },
      ]"
      @click="handleFullTextTranslate"
      :disabled="!serviceOffStore.isServiceOn"
    >
...
const startToPolling = async (
  ...
  try {
    ...
    // 轮询被中断或超过最大尝试次数
    console.error('getTranslateStatus 轮询异常:', error);
    data.value = {
      status: FullTranslateFlowStatus.WITHOUT_TRANSLATE_HISTORY,
    };
    //刷新用户积分
...
const handleFullTextTranslate = async () => {
  if (!serviceOffStore.isServiceOn) return;
...
.full-text-translate-btn:disabled,
.full-text-translate-btn.service-off {
  opacity: 0.5;
  cursor: not-allowed;
  color: var(--site-theme-text-disabled, rgba(0, 0, 0, 0.25));
  border-color: var(--site-theme-divider, #d9d9d9) !important;
}

.full-text-translate-btn:disabled:hover,
.full-text-translate-btn.service-off:hover {
