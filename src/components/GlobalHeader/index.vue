    ...
    <div class="header">
      ...
      <div class="actions-container">
        ...
        <div class="avatar-container">
          <a-dropdown :trigger="['click']" placement="bottomRight" @visible-change="handleAvatarDropdownVisibleChange">
            ...
            <template #overlay>
              <a-menu>
                ...
                <a-menu-divider /> -->
                <a-menu-item v-if="isTauriAnonymous" key="cloudLogin" @click="handleCloudLogin">
                  <CloudOutlined />
                  <span>{{ cloudLoginText }}</span>
                </a-menu-item>
                <a-menu-item v-if="!isTauriAnonymous" key="logout" @click="handleLogout">
                  <LogoutOutlined />
...
import { doLogout, checkCloudLoginStatus } from '~/src/api/user';
...
// Tauri 环境下的匿名用户：隐藏退出登录，显示云端登录
const isTauriAnonymous = computed(() => isInTauri() && userStore.isAnonymous());
const homeUrl = isInElectron() ? `https://${getHostname()}/` : '/';

// 云端登录状态
const cloudLoginStatus = ref<boolean | null>(null);

// 云端登录状态文本
const cloudLoginText = computed(() => {
  if (cloudLoginStatus.value === null) {
    return t('user.cloudLogin'); // 加载中，显示默认文本
  }
  return cloudLoginStatus.value 
    ? t('user.cloudAccountLoggedIn') 
    : t('user.cloudAccountNotLoggedIn');
});

// 检查云端登录状态
const checkCloudStatus = async () => {
  if (!isTauriAnonymous.value) return;
  
  try {
    const isValid = await checkCloudLoginStatus();
    cloudLoginStatus.value = isValid;
  } catch (error) {
    console.error('[GlobalHeader] 检查云端登录状态失败:', error);
    cloudLoginStatus.value = false;
  }
};
...
// 下拉菜单可见性变化时检查云端状态
const handleAvatarDropdownVisibleChange = (visible: boolean) => {
  dropdownVisible.value = visible;
  if (visible && isTauriAnonymous.value) {
    checkCloudStatus();
  }
};
...
const handleLogoClick = (e: MouseEvent) => {
  reportClick(ElementClick.go_home);
  
  // Tauri 模式下使用路由跳转，阻止默认的链接行为
  if (isInTauri()) {
    e.preventDefault();
    window.location.href = homeUrl;
  }
  // 非 Tauri 模式下让 <a> 标签的默认行为生效（target="_blank"）
};
