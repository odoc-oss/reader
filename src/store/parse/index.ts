...
const actions: ActionTree<ParseState, RootState> & Actions = {
  async [ParseActionTypes.GET_PDF_FIGURE_AND_TABLE_INFOS]({ commit }, pdfId) {
    const { useServiceOffStore } = await import('~/src/stores/serviceOffStore');
    if (!useServiceOffStore().isServiceOn) {
      commit(ParseMutationTypes.SET_PDF_FIGURE_AND_TABLE_INFOS, { pdfId, infos: { list: [], pending: false, error: null } });
      return;
    } 
