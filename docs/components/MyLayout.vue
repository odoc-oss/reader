...
const heroData = computed(() => {
  ...
  // 4. 构建指南链接，使用 isCurrentLanguage 判断（在 computed 内部调用，响应语言变化）
  const guideLink = isCurrentLanguage(Language.ZH_CN) ? '/docs/zh/guide' : '/docs/guide';
  
  // 5. 返回最终的数据对象，供 Hero 组件渲染
  return {
    ...
    actions: [
      ...
      // 指南链接，仍然是 VitePress 内部链接，使用 isCurrentLanguage 响应语言变化
      { theme: 'alt', text: translations?.guideText || "User Guide", link: guideLink }
    ]
  }
})
